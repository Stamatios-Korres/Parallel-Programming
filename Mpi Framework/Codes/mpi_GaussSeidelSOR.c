...
int main(int argc, char ** argv) {
	...
	for (t=0;t<T;t++) {
		...
		if(left >= 0){	
			MPI_Irecv(&(u_current[0][0]),1,col_block,left,5,MPI_COMM_WORLD,&requests[counter]);
			counter++;		 				
			MPI_Isend(&(u_previous[0][1]),1,col_block,left,4,MPI_COMM_WORLD,&requests[counter]);	
			counter++;
		}
		if(up>=0){
			MPI_Irecv(&(u_current[0][0]),1,row_block,up,3,MPI_COMM_WORLD,&requests[counter]);
			counter++;
		...
		if(right>=0)
			MPI_Isend(&(u_current[0][local[1]]),1,col_block,right,5,MPI_COMM_WORLD,&requests[counter]);	
		if(down >=0)
			MPI_Isend(&(u_current[local[0]][0]),1,row_block,down,3,MPI_COMM_WORLD,&requests[counter]);

		#ifdef TEST_CONV
		gettimeofday(&tcs,NULL);
		gettimeofday(&tcns,NULL);
		if (t%C==0) {
			converged=converge(u_current,u_previous,local[0]+1,local[1]+1);	
	...
	if(rank!=0)
		ptr = malloc(1);
	MPI_Gatherv(&(u_current[1][1]),1,local_block,ptr,scattercounts,scatteroffset,global_block,0,MPI_COMM_WORLD);
	free(u_current);
	free(u_previous);
	//#define PRINT_RESULTS
	if (rank==0) {
		printf("GaussSeidelSOR  X %d Y %d Px %d Py %d Iter %d ConvergeTime %lf ComputationTime %lf TotalTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t-1,conv_time,comp_time,total_time,U[global[0]/2][global[1]/2]);
		#ifdef PRINT_RESULTS
