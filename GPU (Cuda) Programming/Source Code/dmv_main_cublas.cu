...
//Let the games begin!
    ...
    float *alpha, *beta;
    alpha = (float*) malloc(sizeof(*alpha));
    beta  = (float*) malloc(sizeof(*beta));
    *alpha = 1.0;
    *beta = 0.0;
    ...
    if (kernel == 0)
        printf("GPU kernel version: cuBLAS library\n");
    ...
    for (size_t i = 0; i < NR_ITER; ++i) {
        if(kernel ==1);
            ...
            cublasSgemv(handle, CUBLAS_OP_T, n, n, alpha, gpu_A, n, gpu_x, 1, beta, gpu_y, 1);
        else if(kernel == 2){
              ; 
              // gpu_kernels[kernel].fn<<<gpu_grid,gpu_block,shmem_size>>>(gpu_A, gpu_x, gpu_y, n);
    }
